...
void RTAE1234() { //default mixer 
 ...
 // if (limit < )
  ...
  rcChannels[5]=map(analogRead(PB5), 0, 4095, RC_CHANNEL_MIN, RC_CHANNEL_MAX);
...
void AETR1234() { //default mixer 
 /*
  ...
  roll = map(analogRead(joystick_R), 3460, 920, RC_CHANNEL_MIN, RC_CHANNEL_MAX);  
*/
 throttle = map(analogRead(joystick_T),  0,  4095, RC_CHANNEL_MIN, RC_CHANNEL_MAX);
  yaw = map(analogRead(joystick_Y),  4095,  0, RC_CHANNEL_MIN, RC_CHANNEL_MAX);
