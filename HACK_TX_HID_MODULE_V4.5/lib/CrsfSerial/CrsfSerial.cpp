...
void CrsfSerial::packetChannelsPacked(const crsf_header_t *p)
{
    ...
    lastChannelsPacket = millis();
    if (onPacketChannels)
        onPacketChannels();
}

void CrsfSerial::CommandPacked(const crsf_header_t *p)
{ 
   crsf_command_t *ch = (crsf_command_t *)&p->data;
   // memcpy(&CommPacket, ch, 8);
   CommPacket[0] = ch->elrs_addr;
   CommPacket[1] = ch->addr_radio;
   CommPacket[2] = ch->command;
   CommPacket[3] = ch->val;
   CommPacket[4] = ch->crc;
    if (!_linkIsUp && onLinkUp)
        onLinkUp();
