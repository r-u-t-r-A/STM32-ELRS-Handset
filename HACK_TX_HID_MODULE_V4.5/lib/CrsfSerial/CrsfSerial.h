...
public:
   ...
   // void setPassthroughMode(bool val, uint32_t passthroughBaud = 0);
    unsigned long lastChannelsPacket = 0;
    unsigned long getTimeSincePacket() const {return lastChannelsPacket;}
   ...
   // void (*onPacketGps)(crsf_sensor_gps_t *gpsSensor);
    ...
    void (*onCommandPacket)();
   // int getCommandType() const { return CommPacket[5]; }
    int getCommandValue() const { return CommPacket[3]; }
    int getCommandType() const { return CommPacket[2]; }
private:
   ...
   // uint32_t _passthroughBaud;
    ...
    void CommandPacked(const crsf_header_t *p);
};
